...
Workspace professionnel — 5 onglets (`proView`) :

- 💶 **Économie** — charges/produits/marges, export PDF.
- 🌿 **Phytosanitaire** — registre AMM/dose/DAR/surface, alertes DAR, export PDF DGAL/PAC.
- 📦 **Lots** — traçabilité récolte, passeports phytosanitaires QR SVG.
- 📡 **Capteurs** — sondes sol IoT (HTTP/JSON), endpoint/token/seuils par sujet.
- 👥 **Équipe** — opérateurs, tâches planifiées (voir §4.28).

#### 🌱 Pépinière (`page='nursery'`, `renderNurseryPage()`)

Page de premier niveau, visible uniquement pour `pepinieriste`. 4 onglets (`nursView`) :

- 🌱 **Semis** — registre des semis (espèce, substrat, germination, repiquage).
- ✂️ **Greffage** — vue agrégée `_renderNursGraftView()` : tous les événements greffage de tous les plants avec statuts reprise/disponibilité, lien fiche.
- 📋 **Catalogue** — stock de vente par référence (espèce × variété × porte-greffe × pot), sync automatique depuis les cohortes greffage.
- 🛒 **Commandes** — commandes clients, étiquettes QR SVG inline.
