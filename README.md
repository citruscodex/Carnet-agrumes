...
4. [Modules détaillés](#4-modules-détaillés)
   ...
   - 4.25 [Module Gestion (économique)](#425-module-gestion-économique)
   ...
   - 4.29 [Pépinière & Fiches Greffes](#429-pépinière--fiches-greffes)
...
- **Fichier unique** `index.html` (~1,1 Mo) — copiez-le sur votre appareil, ouvrez-le dans Chrome, Safari ou Firefox, et c'est prêt.
...
- **~520 fonctions JavaScript** couvrant la météo, la botanique, la cartographie, la nutrition végétale et la phénologie lunaire.
...
| Fichier | `index.html` (~1,1 Mo, ~17 870 lignes) |
| Langage | HTML5 + CSS3 + JavaScript ES2022 (vanilla) |
| Stockage | `localStorage` — 12 clés (voir tableau ci-dessous) |
...
### 4.17 Navigation adaptative & pages principales (v8.1)

**Fonctions :** `renderSettings`, `renderProPage`, `renderNurseryPage`, `_renderNursGraftView`, `_updateNavbarForProfile`, `showPage`, `saveProfile`

Depuis la v8.1, la navigation s'adapte au `profileType`. Les outils métier opérationnels sont séparés des réglages personnels.

#### Navigation par profil

| Profil | Barre de navigation |
|---|---|
| 🍋 Collectionneur · 🏛 Conservatoire | 📊 · 🍋 · ＋ · 📅 · ⚙ Réglages |
| 🌳 Arboriculteur | 📊 · 🍋 · ＋ · 📅 · 🌿 Pro |
| 🌱 Pépiniériste | 📊 · 🍋 · 🌱 Pépinière · 📅 · 🌿 Pro |

`_updateNavbarForProfile()` gère la visibilité de `nv-eco`, `nv-pro`, `nv-nursery`, `nv-add`, `nv-settings` sans mutation d'id DOM. Variables d'état : `profView` (réglages), `proView` (espace pro), `nursView` (pépinière).
...
Workspace professionnel. Onglets `proView` — liste adaptée au profil :

- 💶 **Gestion** — charges/produits/marges, export PDF. *(Tous Pro)*
- 🌿 **Phytosanitaire** — registre AMM/dose/DAR/surface, alertes DAR, export PDF DGAL/PAC. *(Tous Pro)*
- 📦 **Lots** — traçabilité récolte, passeports phytosanitaires QR SVG. *(Tous Pro)*
- 📡 **Capteurs** — sondes sol IoT (HTTP/JSON), endpoint/token/seuils par sujet. *(Tous Pro)*
- 👥 **Équipe** — opérateurs, tâches planifiées (voir §4.28). *(Tous Pro)*
- 🌱 **Pépinière** — `renderProfNursery()` — semis, fiches greffes directes, catalogue, commandes. *(Pépiniériste uniquement)*
...
### 4.25 Module Gestion (économique)
...
### 4.29 Pépinière & Fiches Greffes

**Fonctions :** `loadNursery`, `saveNursery`, `getNursery`, `_nurseryGraftSync`, `renderProfNursery`, `_renderSemisView`, `_renderGraftsView`, `_renderCatalogView`, `_renderOrdersView`, `openSemisModal`, `saveSemisModal`, `deleteSemis`, `openGraftSheetModal`, `_buildScionRow`, `_addScionRow`, `saveGraftSheetModal`, `deleteGraftSheet`, `graftSheetToCatalog`, `openCatalogModal`, `saveCatalogModal`, `deleteCatalogItem`, `openOrderModal`, `saveOrderModal`, `deleteOrder`, `deleteHistoryEntry`, `generateNurseryLabelPDF`

Intégré dans 🌿 Pro → onglet 🌱 Pépinière (pépiniériste uniquement). Store `agrumes_nursery` — 5 collections : `semis`, `grafts`, `catalog`, `orders`, `history`.

**Fiches Greffes (`grafts[]`) :** nouveau module de lot de greffage direct. Structure : `{ id, date, rootstockSpecies, rootstockQty, method, status, scions[], notes, createdAt }`. Chaque `scion` : `{ id, species, variety, qtyGrafted, reprisePct, repriseQty, disponibilite }`. Bouton **→ Catalogue** par greffon repris : crée une entrée `catalog` avec `fromGraftSheetId` + `fromScionId`. Barre de progression par fiche (reprises / total greffés). 4 statuts : `en_cours` / `reprise` / `disponible` / `perdu`.

**Historique livraisons (`history[]`) :** chaque commande livrée (status=done) génère une entrée. Boutons ✏️ (ouvre la commande source) et 🗑 (supprime l'entrée + l'écriture éco `_nurseryOrderId`, sans toucher aux stocks).

Sync automatique greffage → catalogue : `_nurseryGraftSync()` propose les cohortes avec `disponibilite` atteinte. Livraison d'une commande décrémente le stock catalogue. Étiquettes PDF : grille 160px, QR SVG maison, espèce/variété/porte-greffe/taille/prix.
...
**Fonctions :** `_syncDeriveKey`, `_syncEncrypt`, `_syncDecrypt`, `_syncCollect`, `_syncApply`, `_gistPush`, `_gistPull`, `_webdavPush`, `_webdavPull`, `_syncConflictDialog`, `_syncGetWdPass`, `_syncSaveWdPass`, `syncPush`, `syncPull`, `syncUIPush`, `syncUIPull`, `syncSaveConfig`, `renderProfSync`
...
**Bundle :** tous les `localStorage` keys sauf secrets (`pwdHash`, `tokenObf`, `claudeKeyObf`, `pnKeyObf`, `pidKeyObf`, `syncWdPassObf`). Config mergée côté client : les secrets locaux sont toujours préservés.
