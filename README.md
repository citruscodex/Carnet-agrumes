...
### 4.13 Greffage — 13e type d'événement

Type `greffage` ajouté à `buildET()` (couleur `#388e3c`, icône `🌿`), `CAL_TYPES`, `typeOrder`.

**Panneau `greffage-wrap`** dans `openEventForm()` :
- Méthode (écusson, fente, en couronne, chip-budding, approche)
- Stade phénologique du greffon (repos, gonflement, débourrement, feuillaison)
- Greffon (autocomplétion `SP[]`) + porte-greffe (autocomplétion `RS_LIST`, pré-rempli depuis `eb.rootstock`)
- Cohorte : `lotId` auto-généré (`LOT-AAAA-MM-XXX`), `lotQty`, `lotSource`
- Reprise : `reprisePct`, `repriseQty` calculé live, `repriseDate`
- Élevage : durée en semaines → `disponibilite` calculée automatiquement

**Fonctions :** `_graftAutoLotId(pid)`, `_graftCalcDispo()`, `_graftCalcReprise()`, `openGraftRepriseModal(plantId, evId)`, `saveGraftReprise(plantId, evId)`

**Données stockées :** `ev.greffon`, `ev.methode`, `ev.stade`, `ev.porteGreffe`, `ev.lotId`, `ev.lotQty`, `ev.lotSource`, `ev.reprisePct`, `ev.repriseQty`, `ev.repriseDate`, `ev.elevageDuree`, `ev.disponibilite` · `plant.lastGreffage`

**Affichage (étapes 3 & 4) :** badges `🌿 greffon → PG`, `LOT-…`, `×50`, `✓ 82%` dans `renderEvEntry()` et dans la vue détaillée du journal · ligne de contexte dans le calendrier mensuel

**`renderGraffageRegistry(plant)`** — section `🌿 Registre des greffages` dans la fiche sujet (après `renderDiagHistory`) : liste tous les lots avec méthode, greffon, taux de reprise, disponibilité colorée, statistiques agrégées (taux moyen, stock en élevage)

**`getGraftAlerts()`** — alertes tableau de bord : reprise à évaluer (J>21 sans `reprisePct`) · disponibilité imminente (<28j)

**Help anchor :** `HELP_CONTENT.greffage` + `helpBtn('greffage')` dans le panneau · section `id="greffage"` dans `aide.html`

### 4.14 Diagnostic — PlantNet, Plant.id, multi-zones

**Pipeline 4 étapes :** A) PlantNet → confirmation espèce · B) Plant.id → diagnostic maladies IA · C) Claude Vision → analyse multimodale · D) heuristique local (toujours actif)

**PlantNet :** `identifyPlantNet(b64jpeg)` — identifie l'espèce botanique, améliore le scoring KB des étapes suivantes. Clé gratuite (500 req/j), stockée chiffrée XOR. Fonctions : `getPlantNetKey()`, `savePlantNetKey()`.

**Plant.id :** `diagnosePlantId(b64jpeg, speciesHint)` + `_parsePlantIdResult(raw, lang)` — diagnostic maladies & ravageurs par modèle IA dédié. Retourne `isHealthyPct`. Clé plan Lite (~100/mois gratuit). Fonctions : `getPlantIdKey()`, `savePlantIdKey()`.

**Multi-zones :** `_diagZones` (Set) remplace `_diagZone` (string). `_diagToggleZone(z)` gère la sélection multiple. Zone `plante_entiere` sélectionne toutes les zones d'un coup. Sélecteur âge feuilles (jeunes/vieilles) pour discriminer carences Fe/Mg. Moteur `diagnoseLocal()` mis à jour : gate `cond.requireZones.some(rz => zones.includes(rz))`, scoring proportionnel au nombre de zones concordantes.
...
| Fichier | `index.html` (~774 Ko, ~13306 lignes) |
| Langage | HTML5 + CSS3 + JavaScript ES2022 (vanilla) |
| Stockage | `localStorage` — 5 clés : `agrumes_v5`, `agrumes_cfg`, `agrumes_fertilizers`, `agrumes_amendments`, `agrumes_verger` |
...
**Fonctions :** `renderProfile`, `renderProfSecurity`, `saveProfile`

**3 onglets :**

🌿 **Profil** — Infos personnelles (nom, localisation, description, ancienneté), culture en pot (orangerie, substrat, source d'eau), pleine terre (sol, exposition, zone climatique, stratégie hivernale), notes générales, données de la collection, export/import JSON, bouton verrouiller.

🔒 **Sécurité** — Statut du mot de passe, type de chiffrement, statut GitHub (connexion, dépôt, photos archivées), boutons modifier token et changer mot de passe.

📋 **Historique** — Chronologie complète de toute la collection (voir section suivante).
